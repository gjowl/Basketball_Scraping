...
def compare_player_scatterplot(_playerdf_1, _playerdf_2, _xaxis, _yaxis, n=0):
    ...
    fig.add_trace(go.Scatter(x=_playerdf_1[_xaxis], y=_playerdf_1[_yaxis], mode='markers', name=player_name_1, hovertemplate=hover_template_1, marker=dict(color='blue', size=10, line=dict(width=2, color='DarkSlateGrey'))))
    fig.add_trace(go.Scatter(x=_playerdf_1[_xaxis], y=_playerdf_1[_yaxis], mode='lines', name=player_name_1, line=dict(color='blue', width=2)))
...
with c2:
    st.write('**Choose a second player**')
...
if player_name == player_name_2:
    ...
    # TODO: add to this section when time
else:
